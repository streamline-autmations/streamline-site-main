...
import GatherBackdrop from '../three/GatherBackdrop';
 ...
 * with the gathering network running behind one huge headline: scattered nodes
 * converge into the automation cluster as the section scrolls in, the cluster
 * leans toward the cursor at rest, and hovering the CTA fires a packet burst.
 * One of only two dark surfaces on the site. On mobile/reduced-motion the
 * canvas doesn't load and the CSS gradient blooms carry the backdrop.
 */
export default function PreFooterCTA({
  ...
  // Incremented on CTA hover; the scene watches it and fires the burst.
  ...
  return (
    <section className="relative flex min-h-[78svh] flex-col items-center justify-center overflow-hidden rounded-t-[2.5rem] bg-site-ink py-28 text-white md:rounded-t-[4.5rem] md:py-32">
      {/* Living gradient backdrop (transform-only drift) — the no-WebGL fallback;
          on desktop the opaque gather canvas paints over it. */}
      ...
      {/* The gathering network — everything coming together behind the ask. */}
      <GatherBackdrop burstSignal={burstSignal} />
      ...
      <div className="relative mx-auto w-full max-w-5xl px-6 text-center">
        ...
        >
          {/* hover = intent-to-click → the network fires a packet burst */}
          <span onPointerEnter={() => { burstSignal.current += 1; }}>
