...
/** By the numbers — honest stats, count up on scroll-in. */
export default function StatsSection() {
  return (
    <Panel bg="ink" className="px-6 py-24 md:px-10 md:py-32">
      <div className="mx-auto w-full max-w-6xl">
        <Tag variant="outline-dark" className="mb-10">
          By the numbers
        </Tag>
          ...
          {STATS.map((s) => (
            <div key={s.label} className="border-t border-white/10 pt-6">
              <CountUp
                ...
                prefix={s.prefix}
                suffix={s.suffix}
                className="block text-[clamp(48px,7vw,84px)] font-semibold leading-none tracking-[-0.03em] text-white"
              />
              <p className="mt-4 max-w-[24ch] text-[14px] leading-[1.5] text-white/60">{s.label}</p>
            </div>
          ))}
        </div>
      </div>
